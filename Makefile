...
SRC_GEMM_GROUPED_LOOP = $(CUDA_DIR)/grouped_gemm_loop.cu
SRC_GEMM_GROUPED_GROUPED = $(CUDA_DIR)/grouped_gemm_grouped.cu
SRC_GEMM_GROUPED_CUSTOM = $(CUDA_DIR)/grouped_gemm_custom.cu
...
TARGET_GEMM_GROUPED_LOOP = $(BIN_DIR)/tmain_grouped_loop
TARGET_GEMM_GROUPED_GROUPED = $(BIN_DIR)/tmain_grouped_grouped
TARGET_GEMM_GROUPED_CUSTOM = $(BIN_DIR)/tmain_grouped_custom
...
all: $(TARGET_FP32) $(TARGET) $(TARGET_GEMM_BATCHED) $(TARGET_GEMM_GROUPED) $(TARGET_GEMM_GROUPED_LOOP) $(TARGET_GEMM_GROUPED_GROUPED) $(TARGET_GEMM_GROUPED_CUSTOM) $(TARGET_GEMM_GROUPED_HALF)
...
$(TARGET_GEMM_GROUPED_LOOP): $(SRC_GEMM_GROUPED_LOOP) $(COMMON_DEPS) $(INCLUDE_DIR)/grouped_gemm_common.cuh | $(BIN_DIR)
	$(CC) $(SRC_GEMM_GROUPED_LOOP) -o $@ $(FLAGS_NVTX)

$(TARGET_GEMM_GROUPED_GROUPED): $(SRC_GEMM_GROUPED_GROUPED) $(COMMON_DEPS) $(INCLUDE_DIR)/grouped_gemm_common.cuh | $(BIN_DIR)
	$(CC) $(SRC_GEMM_GROUPED_GROUPED) -o $@ $(FLAGS_NVTX)

$(TARGET_GEMM_GROUPED_CUSTOM): $(SRC_GEMM_GROUPED_CUSTOM) $(COMMON_DEPS) $(INCLUDE_DIR)/grouped_gemm_common.cuh | $(BIN_DIR)
	$(CC) $(SRC_GEMM_GROUPED_CUSTOM) -o $@ $(FLAGS_NVTX)

$(TARGET_GEMM_GROUPED_HALF): $(SRC_GEMM_GROUPED_HALF) $(COMMON_DEPS) | $(BIN_DIR)
	$(CC) $(SRC_GEMM_GROUPED_HALF) -o $@ $(FLAGS_NVTX)

grouped: $(TARGET_GEMM_GROUPED) $(TARGET_GEMM_GROUPED_LOOP) $(TARGET_GEMM_GROUPED_GROUPED) $(TARGET_GEMM_GROUPED_CUSTOM) $(TARGET_GEMM_GROUPED_HALF) $(TARGET_CUTLASS_GROUPED)

fp32: $(TARGET_FP32)

clean:
	rm -f $(TARGET_FP32) $(TARGET) $(TARGET_GEMM_BATCHED) $(TARGET_GEMM_GROUPED) $(TARGET_GEMM_GROUPED_LOOP) $(TARGET_GEMM_GROUPED_GROUPED) $(TARGET_GEMM_GROUPED_CUSTOM) $(TARGET_GEMM_GROUPED_HALF) $(TARGET_CUTLASS_GROUPED)
