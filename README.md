...
# 3. 빌드 및 실행
make grouped
python experiments/compare_grouped/benchmark_end_to_end.py
```
...
```bash
python experiments/compare_grouped/benchmark_end_to_end.py [--config config.yaml] [--backend cuda|triton|torch|cutlass|all]
```
...
```bash
python experiments/compare_grouped/benchmark_end_to_end.py --nsight [--nsight-out <dir>] [--ncu-extra "-c 100"]
```
...
| CUDA FP16 | grouped_gemm_fp16_all   | Loop+Grouped 한 리포트에 (동일 커널 5개 = Grouped 5회) |
| CUDA FP16 | grouped_gemm_fp16_loop  | Loop만 (cuBLAS 반복)                                  |
| CUDA FP16 | grouped_gemm_fp16_grouped | Grouped만 (cuBLAS Grouped API)                      |
...
```bash
python experiments/compare_grouped/benchmark_kernel_only.py [--config config.yaml]
```
...
│   ├── benchmark_end_to_end.py    end-to-end 백엔드 벤치마크 + Nsight 프로파일링
│   ├── run_multi_config.py    여러 config 순차 실행, config별 결과 저장
│   ├── benchmark_kernel_only.py   단일-커널(연산시간) 백엔드만 비교
