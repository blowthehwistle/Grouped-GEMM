...
__global__ void double_buffering_tf(float *__restrict A, float *__restrict B, float *C, int m, int n, int k, float alpha, float beta) {
...
#pragma unroll
    ...
    for(int offset = 0; offset + strideA <= bm; offset += strideA) {
        cuda::memcpy_async(thread,
                            reinterpret_cast<float4*>(&A_shared[A_trow * bk + A_tcol + offset * bk]),
    ...
    for(int offset = 0; offset + strideB <= bk; offset += strideB) {
        cuda::memcpy_async(thread,
                            reinterpret_cast<float4*>(&B_shared[(B_trow + offset) * bn + B_tcol]),
                            reinterpret_cast<float4*>(&B[(B_trow + offset) * n + B_tcol]),
                            sizeof(float4),
                            pipeline);
    }
...
#pragma unroll
    for(int i = 0; i < k - bk; i += bk) {
        ...
        for(int offset = 0; offset + strideA <= bm; offset += strideA) {
            cuda::memcpy_async(thread,
                                reinterpret_cast<float4*>(&A_shared[(1 - shm_offset) * bm * bk + A_trow * bk + A_tcol + offset * bk]),
        ...
        for(int offset = 0; offset + strideB <= bk; offset += strideB) {
            cuda::memcpy_async(thread,
                                reinterpret_cast<float4*>(&B_shared[(1 - shm_offset) * bk * bn + (B_trow + offset) * bn + B_tcol]),
                                reinterpret_cast<float4*>(&B[(B_trow + offset) * n + B_tcol + bk * n]),
                                sizeof(float4),
                                pipeline);
        }
        pipeline.producer_commit();
        ...
        for(int frag = 0; frag < bk; frag += 8) {
            ...
            for(int innerw_row = 0; innerw_row < wmiter; innerw_row++) {
                wmma::load_matrix_sync(a_frag[innerw_row], A_shared + shm_offset * bm * bk + wrow * wm * bk + frag + innerw_row * 16 * bk, bk);
            }
            #pragma unroll
            for(int innerw_col = 0; innerw_col < wniter; innerw_col++) {
                wmma::load_matrix_sync(b_frag[innerw_col], B_shared + shm_offset * bk * bn + wcol * wn + frag * bn + innerw_col * 16, bn);
            }
