...
KERNEL="include/cuda_kernels/6_vectorized_revised.cuh"
OUTPUT="results/benchmark/6_vectorized_revised.txt"
      ...
      for bm in ${BM_VALUES[@]}; do
        for bn in ${BN_VALUES[@]}; do
          for nt in ${NUM_THREADS_VALUES[@]}; do
            ...
            sed -i "s/const uint k7_bk = .*/const uint k7_bk = $bk;/" $RUNNER
            sed -i "s/const uint k7_tw_m = .*/const uint k7_tw_m = $tm;/" $RUNNER
            sed -i "s/const uint k7_tw_n = .*/const uint k7_tw_n = $tn;/" $RUNNER
            sed -i "s/const uint k7_bm = .*/const uint k7_bm = $bm;/" $RUNNER
            sed -i "s/const uint k7_bn = .*/const uint k7_bn = $bn;/" $RUNNER
