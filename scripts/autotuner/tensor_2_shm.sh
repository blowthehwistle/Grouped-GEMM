...
KERNEL="include/tensor_kernels/2_shm.cuh"
OUTPUT="results/benchmark/tensor_2_shm.txt"
			...
			for WM in "${WM_VALUES[@]}"; do
				for WN in "${WN_VALUES[@]}"; do
					for NUM_THREADS in "${NUM_THREADS_VALUES[@]}"; do
						...
						sed -i "s/const uint t_k2_num_threads = .*/const uint t_k2_num_threads = $NUM_THREADS;/" $RUNNER
						sed -i "s/const uint t_k2_bn = .*/const uint t_k2_bn = $BN;/" $RUNNER
						sed -i "s/const uint t_k2_bm = .*/const uint t_k2_bm = $BM;/" $RUNNER
						sed -i "s/const uint t_k2_bk = .*/const uint t_k2_bk = $BK;/" $RUNNER
						sed -i "s/const uint t_k2_wm = .*/const uint t_k2_wm = $WM;/" $RUNNER
						sed -i "s/const uint t_k2_wn = .*/const uint t_k2_wn = $WN;/" $RUNNER
