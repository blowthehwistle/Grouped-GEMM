...
KERNEL="include/tensor_kernels/4_shm_bypass.cuh"
OUTPUT="results/benchmark/tensor_4_shm_bypass_16k_t1.txt"
			...
			for WM in "${WM_VALUES[@]}"; do
				for WN in "${WN_VALUES[@]}"; do
					for NUM_THREADS in "${NUM_THREADS_VALUES[@]}"; do
						...
						sed -i "s/const uint t_k4_num_threads = .*/const uint t_k4_num_threads = $NUM_THREADS;/" $RUNNER
						sed -i "s/const uint t_k4_bn = .*/const uint t_k4_bn = $BN;/" $RUNNER
						sed -i "s/const uint t_k4_bm = .*/const uint t_k4_bm = $BM;/" $RUNNER
						sed -i "s/const uint t_k4_bk = .*/const uint t_k4_bk = $BK;/" $RUNNER
						sed -i "s/const uint t_k4_wm = .*/const uint t_k4_wm = $WM;/" $RUNNER
						sed -i "s/const uint t_k4_wn = .*/const uint t_k4_wn = $WN;/" $RUNNER
						...
						./bin/tmain_16k 4 0 | tee -a $OUTPUT
